...
curl -X PATCH "https://talkover.app/api/v1/campaigns/campaign-uuid-1/status" \
...
```javascript
// Request 1: Activate a draft campaign
const response = await fetch('https://talkover.app/api/v1/campaigns/campaign-uuid-1/status', {
  method: 'PATCH',
  headers: {
