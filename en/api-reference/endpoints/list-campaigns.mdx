...
curl -X GET "https://talqover.app/api/v1/campaigns" \
...
curl -X GET "https://talqover.app/api/v1/campaigns?status=active&campaign_type=sales&per_page=50" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Request 2: Get active sales campaigns
const response = await fetch('https://talqover.app/api/v1/campaigns?status=active&campaign_type=sales&per_page=50', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
...
curl -X GET "https://talqover.app/api/v1/campaigns?search=Sales Campaign&agent_id=agent-uuid-1" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Request 3: Search for specific campaign
const response = await fetch('https://talqover.app/api/v1/campaigns?search=Sales Campaign&agent_id=agent-uuid-1', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
