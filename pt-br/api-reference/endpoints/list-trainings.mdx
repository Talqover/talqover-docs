...
curl -X GET "https://app.talkover.ai/api/v1/agents/agent-uuid-1/trainings" \
...
curl -X GET "https://app.talkover.ai/api/v1/agents/agent-uuid-1/trainings?per_page=50&page=1" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 2: Obter treinamentos com paginação
const response = await fetch('https://app.talkover.ai/api/v1/agents/agent-uuid-1/trainings?per_page=50&page=1', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
