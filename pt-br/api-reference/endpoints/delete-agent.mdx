...
curl -X DELETE "https://talkover.app/api/v1/agents/agent-uuid-1" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 1: Excluir um agente específico
const response = await fetch('https://talkover.app/api/v1/agents/agent-uuid-1', {
  method: 'DELETE',
  headers: {
...
const response = await fetch(`https://talkover.app/api/v1/agents/${agent_id}`, {
  method: 'DELETE',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
