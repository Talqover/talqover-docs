...
curl -X GET "https://talkover.app/api/v1/agents?status=published&language=pt-BR&per_page=50" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 2: Obter agentes publicados em português
const response = await fetch('https://talkover.app/api/v1/agents?status=published&language=pt-BR&per_page=50', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
...
curl -X GET "https://talkover.app/api/v1/agents?search=Atendimento" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 3: Pesquisar por nome específico
const response = await fetch('https://talkover.app/api/v1/agents?search=Atendimento', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
