...
curl -X GET "https://talqover.app/api/v1/calls?status=completed&direction=outbound&per_page=50" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 2: Filtrar por chamadas completadas
const response = await fetch('https://talqover.app/api/v1/calls?status=completed&direction=outbound&per_page=50', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
...
curl -X GET "https://talqover.app/api/v1/calls?date_from=2024-01-01&date_to=2024-01-31&agent_id=agent-uuid-1" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 3: Filtrar por período específico
const response = await fetch('https://talqover.app/api/v1/calls?date_from=2024-01-01&date_to=2024-01-31&agent_id=agent-uuid-1', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
