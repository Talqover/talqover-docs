...
api: "PATCH /api/v1/campaigns/{campaign}/status"
---
...
```
PATCH /api/v1/campaigns/{campaign}/status
```
...
curl -X PATCH "https://talqover.app/api/v1/campaigns/campaign-uuid-1/status" \
...
```javascript
// Requisição 1: Ativar uma campanha
const response = await fetch('https://talqover.app/api/v1/campaigns/campaign-uuid-1/status', {
  method: 'PATCH',
  headers: {
