...
api: "GET /api/v1/agents/{agent}/actions"
---
...
```
GET /api/v1/agents/{agent}/actions
```
...
curl -X GET "https://talqover.app/api/v1/agents/agent-uuid-1/actions" \
...
curl -X GET "https://talqover.app/api/v1/agents/agent-uuid-1/actions?type=webhook&per_page=50" \
  -H "Authorization: Bearer talq_your_environment_token_here"
```

```javascript
// Requisição 2: Filtrar por tipo de ação
const response = await fetch('https://talqover.app/api/v1/agents/agent-uuid-1/actions?type=webhook&per_page=50', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer talq_your_environment_token_here'
  }
