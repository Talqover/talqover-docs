...
Todas as solicitações para a API Talqover requerem autenticação usando tokens de ambiente. Este guia explica como obter seu token e usá-lo em solicitações da API.
...
```cmd
set TALQOVER_TOKEN=talq_seu_token_de_ambiente_aqui
```

### Windows (PowerShell)

```powershell
$env:TALQOVER_TOKEN="talq_seu_token_de_ambiente_aqui"
```
